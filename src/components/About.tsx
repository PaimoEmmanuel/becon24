...
import { aboutTargets } from "../utils/data";
...
const About = () => {
  useEffect(() => {
    ...
    const beconWrapper = document.querySelector(".about-becon-wrapper");

    const handleScrollOne = () => {
      targetLinks.forEach((link, index) => {
        if (isElementInViewport(link, 100)) {
  ...
  useEffect(() => {
    const buttonWrap = document.querySelectorAll(".calendar-wrapper");

    const calendarButtons = document.querySelectorAll("add-to-calendar-button");
    document.addEventListener("mousemove", (event) => {
      buttonWrap.forEach((button, index) => {
        const isClickInside = button?.contains(event.target as Node);
        if (!isClickInside && calendarButtons[index]) {
          calendarButtons[index].style.display = "none";
        }
      });
    });
  }, []);
  ...
  useEffect(() => {
    const modalWrappers = document.querySelectorAll<HTMLElement>(
    ...
    if (modalWrappers.length) {
      modalWrappers.forEach((modal) => {
        modal.addEventListener("click", (e) => {
          const modalContent =
            modal.querySelector<HTMLElement>(".target-modal");
  ...
  return (
    <div id="about-becon" className="about-container">
      ...
      <div className="about-becon-wrapper">
        <div className="about-becon">
          <div>
            {aboutTargets.map((target, index) => (
              <Fragment key={target.title}>
                <div className="target-wrapper">
                  ...
                  <div className="target-links calendar-wrapper">
                    {target.contact ? (
                      <button
                        className="target-know"
                        onClick={() => {
        ...
        c-10.194,0-18.437,10.179-18.437,22.759C126.596,502.71,134.838,512.89,145.032,512.89z"
                ...
                <div className="target-modal-wrapper">
                  <div className="target-modal">
                    ...
                    <div className="target-modal-text">
                      <p className="target-modal-title">{target.title}</p>
                      <p className="target-modal-body">
                        To know more please contact the number below, this is
                        also a WhatsApp number for enquiries.
                      </p>
                      <a
                        href={`tel:${target.contact}`}
                        className="target-modal-contact"
                      >
                        {target.contact}
                      </a>
                    </div>
